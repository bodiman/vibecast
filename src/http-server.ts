...
async function handleLoadModel(args: any) {
  const { name } = args;
...
async function handleSaveModel(args: any) {
  if (!currentModel) {
...
async function handleCreateVariable(args: any) {
  if (!currentModel) {
...
async function handleEvaluateModel(args: any) {
  if (!currentModel) {
    throw new Error('No current model. Create or load a model first.');
  }
