...
export class ModelitMCPServer {
  ...
  constructor(databaseUrl: string) {
    ...
    this.graphDiffService = new GraphDiffService(this.dbStorage);
    ...
    this.server.setRequestHandler(ListToolsRequestSchema, async () => ({
      tools: [
        ...
        },
        {
          ...
          description: 'Generate a diff between current graph state and new graph data with validation',
          inputSchema: {
            type: 'object',
            properties: {
              modelName: { type: 'string', description: 'Model name to compare against' },
              graphData: { 
                type: 'object', 
                description: 'New graph structure to compare',
                properties: {
                  model: {
                    type: 'object',
                    properties: {
                  ...
                  nodes: {
                    ...
                    items: {
                      ...
                      properties: {
                        id: { type: 'string' },
                        name: { type: 'string' },
                        type: { type: 'string', description: 'Node type (scalar, series, parameter)' },
                        formula: { type: 'string' },
                        values: { type: 'array', items: { type: 'number' } },
                        metadata: { type: 'object' },
              ...
              options: {
                type: 'object',
                properties: {
                  validateOnly: { type: 'boolean', description: 'Only validate without generating diff' },
                  includeWarnings: { type: 'boolean', description: 'Include warnings in validation' },
                  skipCycleDetection: { type: 'boolean', description: 'Skip cycle detection for performance' },
                  compareMode: { type: 'string', enum: ['strict', 'lenient'], description: 'Comparison mode' }
                }
              }
            },
            required: ['modelName', 'graphData']
          },
        },
        {
          name: 'apply_graph_diff',
          description: 'Apply a generated graph diff to the database with transaction support',
          inputSchema: {
            type: 'object',
            properties: {
              diff: { 
                type: 'object',
                description: 'Graph diff generated by generate_graph_diff tool'
              },
              options: {
                type: 'object',
                properties: {
                  dryRun: { type: 'boolean', description: 'Simulate the changes without applying them' },
                  createBackup: { type: 'boolean', description: 'Create backup before applying changes' },
                  continueOnError: { type: 'boolean', description: 'Continue applying changes even if validation fails' },
                  batchSize: { type: 'number', description: 'Batch size for large updates' }
                }
              }
            },
            required: ['diff']
      ...
      try {
        switch (name) {
          ...
          case 'generate_graph_diff':
            return await this.handleGenerateGraphDiff(args);
          case 'apply_graph_diff':
            return await this.handleApplyGraphDiff(args);
          default:
...
${cycles.length > 0 ? `Cycles found:\n${cycles.map(cycle => `- ${cycle.join(' → ')}`).join('\n')}` : 'No cycles detected'}`;
  ...
  // New graph diff handlers
  private async handleGenerateGraphDiff(args: any) {
    const { modelName, graphData, options = {} } = args;
    
    try {
      const diff = await this.graphDiffService.generateDiff(modelName, graphData, options);
      ...
      throw new Error(`Failed to generate graph diff: ${error instanceof Error ? error.message : String(error)}`);
    }
  }

  private async handleApplyGraphDiff(args: any) {
    const { diff, options = {} } = args;
    
    try {
      const result = await this.graphDiffService.applyDiff(diff, options);
      
      const summary = `Graph diff ${result.success ? 'successfully applied' : 'failed'}:
- Nodes: ${result.appliedChanges.nodesAdded} added, ${result.appliedChanges.nodesModified} modified, ${result.appliedChanges.nodesDeleted} deleted
- Edges: ${result.appliedChanges.edgesAdded} added, ${result.appliedChanges.edgesModified} modified, ${result.appliedChanges.edgesDeleted} deleted`;
      ...
      if (result.rollbackInfo?.canRollback) {
        responseText += `\nRollback available: ${result.rollbackInfo.rollbackId}`;
      }
